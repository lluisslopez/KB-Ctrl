...
@IonicPage()
@Component({
	selector: 'page-box-form',
	templateUrl: 'box-form.html',
})
export class BoxForm {
	...
	getInformation( user , pass , url){
		...
		let urls =  url + "/api/data/collections/name/C_Coins";
	...
	saveDocument(){
		...
		this.saveDocumentHttp(objSave , "C_Coins_Form", comp);
	}
			...
			if(sendVal){
				if ( this.userOne.length > 0 ) {
					...
					let objOneSignal = {
						...
						"contents": {"en": "Coins format was created and required complete the minimum quantity." },
