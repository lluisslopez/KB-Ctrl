...
@IonicPage()
@Component({
	selector: 'page-box-count',
	templateUrl: 'box-count.html',
})
export class BoxCount {
	...
	ionViewDidLoad() {
		console.log('ionViewDidLoad BoxCount');
	}
	...
	getInformation( user , pass , url){
		...
		let urls =  this.url + "/api/data/collections/name/" + "C_Count_Form";
	...
	prelogin(){
		this.nav.push( "BoxCountForm" , {user: this.user, pass: this.password , url: this.url } );
	}

	view(id, date){
		console.log(id);
		this.nav.push( "BoxCountDo" ,  {id : id , date : date, user: this.user, pass: this.password , url: this.url } );
	}
		...
		this.saveDocumentHttp(objSave , id , "--");
	}
